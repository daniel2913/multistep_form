...
function Variant({ item, className, onClick, active, yearly }: VariantProps) {
	return (
		<button
			type="button"
			onClick={onClick}
			className={`flex lg:flex-col  lg:gap-2 w-full gap-4 p-2 lg:p-4 items-center lg:items-start border rounded-md ${(active && "bg-p-blue-light border-p-blue-marine") ||
		...
		>
			...
			<span
				className={`text-p-blue-marine font-semibold ml-auto lg:ml-0 ${(!yearly.value && "invisible") || ""
					}`}
			>
				2 months free
			</span>
		...
		<>
			...
			<div className="w-full flex justify-center items-center">
				...
				>
					<span>Monthly</span>
					<div
						className={`relative w-full rounded-3xl ${yearly.value ? "bg-p-blue-purple" : "bg-n-gray-cool"
							}`}
					>
						<input
							className="peer"
